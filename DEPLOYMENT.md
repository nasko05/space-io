# Deploying SpaceIO · Hearth (production)

Hearth is co-hosted on the **same Contabo VPS** as the sibling
...
```
              ...
              │  Let's Encrypt   │                │
              └───┬─────────┬────┘                │
   app:8000 ──────┘         └────── hearth:7777 ──┘
 (cloud-storage,        (THIS app, no published 80/443;
  its own project)       loopback 127.0.0.1:8001 for health only)
```

- The sibling app's Caddy is the **single TLS listener**. Hearth runs **no
  Caddy** and **never binds 80/443**.
- Caddy reaches Hearth by container name over the **external `web`** docker
  network. Both Caddy and Hearth attach to it.
- Hearth additionally publishes **`127.0.0.1:8001:7777`** (loopback only) so the
...
| Resource          | cloud-storage-system                                   | Hearth (this app) |
|-------------------|--------------------------------------------------------|-------------------|
| Compose project   | `cloud-storage-system`                                 | `hearth`          |
| Containers        | `db`, `app`, `caddy`, `clamav`                         | `hearth`          |
| Public ports      | `caddy` → 80, 443                                       | none              |
| Loopback port     | `app` → 127.0.0.1:**8000**                              | 127.0.0.1:**8001** |
| Named volumes     | `db_data`, `blob_data`, `backup_data`, `clamav_data`, `caddy_data`, `caddy_config` | `hearth-data` |
| Networks          | project default (+`web` after the wiring below)         | external `web`    |

Hearth has **no database** (it is a filesystem vault) and **no app-wide signing
...
and add a site block for Hearth. This is the only cross-repo change; apply it
...
services:
  caddy:
    ...
    networks:
      ...
      - web          # add:  this is how Caddy reaches Hearth
...
personal-area.personal-drive-io.com {
	reverse_proxy hearth:7777
}
```

After redeploying the sibling app, Caddy will provision a Let's Encrypt cert for
`personal-area.personal-drive-io.com` automatically and proxy it to Hearth.
...
| `HEARTH_OPENROUTER_API_KEY` | ➖       | Turns the in-app AI assistant on. Omit → assistant stays hidden.    |
| `HEARTH_BRAVE_API_KEY`      | ➖       | Direct Brave web search. Omit → OpenRouter's built-in web plugin.   |

**Variables** (non-sensitive; each has a workflow default via `${{ vars.X || '…' }}`):

| Name                     | Default                              | What                                              |
|--------------------------|--------------------------------------|---------------------------------------------------|
| `DOMAIN`                 | `personal-area.personal-drive-io.com`| The subdomain the shared Caddy routes to Hearth.  |
| `HEARTH_AGENT_MODEL`     | `qwen/qwen3.6-27b`                    | Any tool-calling OpenRouter model id.             |
| `HEARTH_AGENT_WEB_SEARCH`| `1`                                  | `0` disables agent web search.                    |
| `HEARTH_AGENT_MAX_STEPS` | `8`                                  | Max tool rounds per agent message.                |
...
User data is the **`hearth-data`** volume (per-user git repos of encrypted
...
0 3 * * *  RCLONE_REMOTE=b2:my-bucket/hearth /home/deploy/space-io/scripts/backup.sh >> /home/deploy/hearth-backup.log 2>&1
```
...
- **Container DNS / ACME fix.** Ubuntu's `systemd-resolved` hands containers a
  ...
  undo it**. The Hearth service also pins those resolvers so the AI agent's
  outbound HTTPS resolves regardless.
- **Forwarded headers / HTTPS behind the proxy.** Hearth speaks plain HTTP
  ...
  fix — the FastAPI sibling needs `FORWARDED_ALLOW_IPS=*` for uvicorn; Hearth's
  axum server needs no equivalent.) Do **not** set `HEARTH_INSECURE_COOKIES` in
  production.
...
- **No port/volume/network collisions.** Hearth uses project `hearth`, container
  `hearth`, volume `hearth-data`, loopback `8001`, and the shared `web` network —
