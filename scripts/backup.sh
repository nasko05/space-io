#!/usr/bin/env bash
# SpaceIO — off-the-container backup of the vault volume.
#
# All user data (per-user git repos of encrypted .age blobs + .users.toml) lives
# in the Docker named volume `space-io-data`. This script snapshots that volume to
...
#   0 3 * * *  /home/deploy/space-io/scripts/backup.sh >> /home/deploy/space-io-backup.log 2>&1
#
# Tunables (env vars, all optional):
#   SPACEIO_VOLUME     Docker volume to back up        (default: space-io-data)
#   BACKUP_DIR        host dir for local snapshots     (default: ~/space-io-backups)
#   BACKUP_RETENTION  how many local snapshots to keep (default: 14)
#   RCLONE_REMOTE     rclone target, e.g. b2:my-bucket/space-io (default: unset → off-site skipped)

set -euo pipefail

SPACEIO_VOLUME="${SPACEIO_VOLUME:-space-io-data}"
BACKUP_DIR="${BACKUP_DIR:-$HOME/space-io-backups}"
...
if ! docker volume inspect "$SPACEIO_VOLUME" >/dev/null 2>&1; then
  echo "error: docker volume '$SPACEIO_VOLUME' does not exist" >&2
  exit 1
fi

mkdir -p "$BACKUP_DIR"
stamp="$(date -u +%Y%m%dT%H%M%SZ)"
archive="$BACKUP_DIR/space-io-${stamp}.tar.gz"

echo ">> snapshotting volume '$SPACEIO_VOLUME' → $archive"
